...
trait IntermediateTranslation extends Acceptable {
  def translate(edge:CFAEdge,originalStatement:Statement): List[CFAEdge]

  def translateForFormula(edge:CFAEdge,originalStatement:Statement): List[CFAEdge] = translate(edge,originalStatement)
}
