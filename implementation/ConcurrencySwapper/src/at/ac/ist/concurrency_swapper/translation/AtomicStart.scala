...
object AtomicStart extends FinalTranslation {
  def translatePoirot(edge: CFAEdge,originalStatement:Statement) = printEdge(edge).replaceFirst("atomicStart","corral_atomic_begin")

  def accepts(edge: CFAEdge):Boolean = {
    ExpressionHelpers.getFunctionDef(edge) match {
      case None => false
      case Some((name,_)) => name == "atomicStart"
    }
  }
