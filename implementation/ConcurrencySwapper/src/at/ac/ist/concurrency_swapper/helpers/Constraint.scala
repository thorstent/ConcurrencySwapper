...
import at.ac.ist.concurrency_swapper.structures.Function

sealed abstract class Constraint[T]

case object SequentialOrder extends Constraint
case class After[T] (first: T, second: T) extends Constraint[T] {
  def this(fs:(T,T)) {
    this(fs._1,fs._2)
  }
}
case class PlaceAtomicSectionFunction[T] (f:Function) extends Constraint[T]
