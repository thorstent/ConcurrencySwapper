...
void sysfs_thread() { int thread_id = corral_getThreadID();
  ...
  while (1) {
    ...
    POIROT_ASSERT(!((((rtnl_lock) == (thread_id)) && ((rtnl_lock_waiting) == (sysfs_lock))) && ((sysfs_lock) != (0))));
    corral_atomic_end();
    corral_atomic_begin();
    ...
    POIROT_ASSERT(!((((rtnl_lock) == (thread_id)) && ((rtnl_lock_waiting) == (dev_lock))) && ((dev_lock) != (0))));
    corral_atomic_end();
    corral_atomic_begin();
...
void drv_reset_task() { int thread_id = corral_getThreadID();
  rtnl_lock_waiting = thread_id;
  corral_atomic_begin();
  POIROT_ASSERT(!((((sysfs_lock) == (thread_id)) && ((sysfs_lock_waiting) == (rtnl_lock))) && ((rtnl_lock) != (0))));
  POIROT_ASSERT(!((((dev_lock) == (thread_id)) && ((dev_lock_waiting) == (rtnl_lock))) && ((rtnl_lock) != (0))));
  corral_atomic_end();
  corral_atomic_begin();
